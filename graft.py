...
def graft(src, dst, extra, progress=None, chunk=CHUNK):
    ...
    # file is held in memory; builds dst + ".partial" and resumes it when a previous run was interrupted
    n, header = read_header(src)
    data_len = max((v["data_offsets"][1] for k, v in header.items() if k != "__metadata__"), default=0)
    offset = data_len
    for k, (dtype, shape, data) in extra.items():
        if k in header:
            raise ValueError(f"source already has {k}")
        header[k] = {"dtype": dtype, "shape": shape, "data_offsets": [offset, offset + len(data)]}
        offset += len(data)
